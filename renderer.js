...
// ============================================================
//  5. ПОДСКАЗКИ ПОИСКА (СДВИГАЮТ WebContentsView)
// ============================================================
...
// ============================================================
//  6. НАСТРОЙКИ (ЧЕРЕЗ IPC, ЕДИНЫЙ ИСТОЧНИК)
// ============================================================
...
async function loadSettingsFromMain() {
  try {
    ...
    if (settings) {
      // Применяем поисковик
      const engine = settings.searchEngine || 'yandex';
      SettingsManager._data.searchEngine = engine;
...
// Обработчики изменения настроек (вместо localStorage)
searchEngineRadios.forEach(radio => {
...
// Подписываемся на изменения настроек из main
...
newTabBtn.addEventListener('click', async () => {
  const tabId = await api.createTab('about:newtab');
